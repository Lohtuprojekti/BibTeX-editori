...
public class ReferenceEntryBuilderTest {
    ...
    @Test
    public void convertScandicsWorks() {
        BibTeXEntry entry = new BibTeXEntry(new Key("Article"), new Key("test"));
        entry.addField(new Key("test"), new KeyValue("\\\"{a}\\\"{a}kk\\\"{o}si\\\"{a}"));
        assertEquals("ääkkösiä", ReferenceEntryBuilder.convertScandic(entry).getField(new Key("test")).toUserString());
    }
}
